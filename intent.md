...
Ember then strengthened the default: disconnect should immediately interrupt ongoing tools and other work, supporting a **disconnect-as-circuit-breaker** mental model. An explicit soft-connection mode should also exist; ember clarified that soft disconnect means **keep working within the existing authority and budget**. Soft attachment does not enlarge either. The [circuit-breaker investigation](sprints/2026-09-19/connection-breaker.md) supersedes the earlier proposal to simply let active work finish while preventing the next step.

## Implementation pause and deeper core investigation (September 19)

Ember authorized an agent-grain implementation swarm, then rejected its introduction of Python as a new implementation language: “let's not start now :) let's discuss what's up.” The instruction to “investigate before we swarm up again” pauses that implementation direction and preserves its WIP for review. It does not direct a mechanical translation of the Python design into Rust. Existing hub tooling is a separate, already established use of Python.

Ember wants us to improve functionality that is currently only apparent, and specifically reopened the possibility of deploying Mini's kernel/node. The old Bread kernel is not selected by default, and the prior agent-selected Mini path is not a settled user decision. Ember also cautioned that old DreggCloud House machinery is substantially independent and needs severe reevaluation before reuse.

The subsequent explicit request was: “do a microswarm on breadstuffs & mini, lets orient deeper and understand what we should actually build up.” That authorizes the bounded investigation recorded in [the core orientation](sprints/2026-09-19/core-orientation.md), not a renewed implementation fleet. Hosted Hermes, canonical programmability, hard/soft disconnect semantics, and the other accepted product goals remain in force.

After that investigation, ember clarified the direction: rebuild into Mini what the New World needs, avoiding an open-ended choice/reuse morass. Bread's machinery can be rebuilt. Mini is now the implementation home; Bread is reference material and optional reuse does not control the architecture or precede construction. See [the decision](sprints/2026-09-19/rebuild-in-mini.md).
